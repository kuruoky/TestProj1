...
class NewLossFunc(nn.Module):
    def __init__(self):
        super(NewLossFunc, self).__init__()
        return

    def forward(self, y_pred_cls, y_true_geo, y_pred_geo):
        h2, w2 = -9999999
        h1, w1 = 9999999
        for tar in y_true_geo:
            h22, w22, h11, w11 = torch.split(tar, 1, 1)
            h1 = min(h1, h11)
            w1 = min(w1, w11)
            h2 = max(h2, h22)
            w2 = max(w2, w22)
        area_true = (h2 - h1) * (w2 - w1)
        hh2, ww2, hh1, ww1 = torch.split(y_pred_cls)
        area_pre = (hh2 - hh1) * (ww2 - ww1)
        area_in = (min(hh2, h2) - max(hh1, h1)) * (min(ww2, w2) - max(ww1, w1))
        los = -torch.log((area_in + 1) / (area_pre + area_true - area_in + 1))
        return los
